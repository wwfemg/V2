...
if ! check_step_done "install_caddy"; then
    ...
    apt update -y || error_exit
    apt install -y caddy=2.7.5 || error_exit
...
if ! check_step_done "install_go"; then
    ...
    ~/go/bin/xcaddy build v2.7.5 --with github.com/caddyserver/forwardproxy@caddy2=github.com/klzgrad/forwardproxy@naive || error_exit
