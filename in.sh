...
if ! check_step_done "install_go"; then
    ...
    if [ "$(uname -m)" == "x86_64" ]; then
      wget https://go.dev/dl/go1.21.2.linux-amd64.tar.gz || error_exit
      rm -rf /usr/local/go && tar -C /usr/local -xzf go1.21.2.linux-amd64.tar.gz || error_exit
    else
      wget https://go.dev/dl/go1.21.2.linux-arm64.tar.gz || error_exit
      rm -rf /usr/local/go && tar -C /usr/local -xzf go1.21.2.linux-arm64.tar.gz || error_exit
    fi
